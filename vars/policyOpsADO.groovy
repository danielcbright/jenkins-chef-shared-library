def call() {
  ...
  stages {
        ...
        stage('Upload to GCS') {
          steps {
            dir("to_upload") {
              // GCS
              googleStorageUpload(
                credentialsId: 'gcs-policyfile-archive',
                bucket: "gs://policyfile-archive/$POLICY_NAME/$POLICY_ID/",
                pattern: "*.*"
              )
            }
          }
        ...
        stage('Upload to S3') {
          steps {
            dir("to_upload") {
              ...
              withAWS(credentials: 'aws-policyfile-archive', region: 'us-east-1') {
                s3Upload(
                  bucket: 'dcb-policyfile-archive',
                  path: "$POLICY_NAME/$POLICY_ID/",
                  includePathPattern: '*.*'
                )
              }            
            }
          ...
          steps {
            dir("to_upload") {
              ...
              azureUpload(
                ...
                containerName: 'policyfile-archive',
                virtualPath: "$POLICY_NAME/$POLICY_ID/"
              )   
            }
          }
        }
    ...
    stage('Kick off Publish Job') {
      steps {
        build 
          job: 'policyfile-publish-PFP/master',
          propagate: false,
          parameters: [
