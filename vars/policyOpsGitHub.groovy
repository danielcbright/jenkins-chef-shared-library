...
def call() {
  ...
  pipeline {
    agent any
    parameters {
        string(defaultValue: 'NOTDEFINED', name: 'ghprbPullLink', description: "${ghprbPullLink}")
    }
    environment {
        HOME = '/root/'
    }
    stages {
      stage('Get Files from GitHub') {
        steps {
          script {
            currentBuild.displayName = "${BUILD_NUMBER} - ${params.ghprbPullLink}"
          }
          checkout([
            $class: 'GitSCM',
            ...
            userRemoteConfigs: [[
              ...
              url: "${params.ghprbPullLink}"
            ]],
          ])
          sh 'ls -alt'
          echo "Build.Repository.Uri: ${params.ghprbPullLink}"
        }
      }
      stage('Tests') {
        steps {
